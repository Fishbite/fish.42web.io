<section class="intro">
  <header id="header">
    <h1 class="title-main satisfy-regular"><?=$title?></h1>

    <div class="intro">
      <p>Fishbite:- It helps if you understand how successful businesses run</p>
      <p>
        A wide spectrum of experience provides us with a fountain of knowledge
      </p>
    </div>
  </header>
</section>

<main class="">
  <section class="intro">
    <h2 class="title-h2">My Background</h2>
    <ul class="list">
      <li class="listElement">customer service</li>
      <li class="listElement">precision engineering</li>
      <li class="listElement">manufacturing</li>
      <li class="listElement">product design</li>
      <li class="listElement">colour management</li>
    </ul>
  </section>

  <div class="bg-container" id="bg-container">
    <img
      id="bg-target"
      class="bg-img"
      src="images/dolphin-thin.svg"
      alt="dolphin thin"
    />
  </div>
  <section id="bg-text-container" class="bg-text">
    <h1 id="bg-title" class="visible">Customer Service</h1>

    <aside id="bg-text">
      <p>There was an error fetchin' data</p>
    </aside>
  </section>

  <section class="txt-container">
    <article class="txt-content">
      <p class="para">
        Created <em>stuff</em> for the Internet since around AD 2000 when I
        first had the idea of taking a stand-alone piece of software written for
        MS Windows&trade; onto the World Wide Web! These were fairly early days
        for the web and everything, browsers, JavaScript, CSS were in their
        infancy. The software in question was used to measure the printing
        characteristics of commercial printing presses, and subsequently provide
        a <em>"correction"</em> that made the press print to a defined standard.
      </p>

      <p class="para">
        Employing and working with programmers to do the coding that made our
        applications 'do their stuff' and I used a very early product call
        'Frogbuilder'&trade; to build application front ends for the printing
        industry. It was created by a commpany by the name of Frogtrade&trade;
        who now specialise in eductional software:
        <a href="https://www.frogeducation.com/" target="_blank"
          >frogeducation.com</a
        >
      </p>

      <p class="para">
        The applications we built were used by commercial printers to guarantee
        that their clients' final printed product would be the exact colours
        they were expecting. This was really cutting edge stuff as it was, but
        making it web based took it to a whole new level giving huge
        organisations visibility and control over the products they were
        producing. All in a browser!
      </p>

      <p class="para">
        Before that, I was managing the development and implementations of
        quality control software in manufacturing environments.
      </p>
    </article>
  </section>
</main>
